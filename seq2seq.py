...
class GRU(object):
    ...
    def process_sequence(self, sequence, h_0 = None):
        ...
        if (h_0 is None):
            ...
            #self.h_0 = tf.convert_to_tensor(h_0, dtype=tf.float64, name = 'h_0')
            self.h_0 = h_0
    ...
    def predict_model(self,sequence, h_0):
        ...
        def loop_cond(prediction, predict_sentence):
            ...
            threshold = tf.constant(0.1, dtype = tf.float64, name = 'whileloop_threshold') 
        ...
        def loop_body(prev_prediction, prev_predict_sentence):
            ...
            next_predict_sentence = tf.concat(axis = 1, values = [prev_prediction, prev_predict_sentence], name = 'whileloop_next_prediction_sentence') 
        ...
        #pred_predicted_output = tf.convert_to_tensor(predict_sentence, dtype=tf.float64, name = 'predict_output')
...
input_sentence = tf.placeholder(dtype=tf.float64, shape=[embedding_dim,None], name='input_data') #sentence_matrix = emb_dim x n_words
...
thought_vector = tf.convert_to_tensor(input_response[:,-1], name = 'thought_vector') #Extract the last state vector (thought) from the input response
...
pred_predicted_output = tf.convert_to_tensor(pred_output_response, dtype=tf.float64, name = 'predict_output')
